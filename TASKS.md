...
// Your solution
The first error that I see is that 'ANlMALS' is not defined. The error is located at `server/api/animals.get.ts`.

The other error that I noticed is that in app.vue there is a call to an `alert` method which is not defined anywhere.
...
// Your solution
I solved the first problem by changing 'ANlMALS' to 'ANIMALS' which is the proper spelling of the variable being created in the line above.

The second issue was fixed by switching the 'alert' method to 'console.log' as I inferred that this is a statement we want to log onto the console. In JavaScript `console.log` is the proper way to log things on the terminal, not `alert`.

Description of the loaded page: There is a small introductory paragraph with information on who to contact for support. The rest of the page includes a table of descriptive information for all the animals of the zoo - important information for a zoo keeper!
